...
const Navbar = () => {
  ...
  return (
    <div className="absolute z-11 w-full bg-[url(/grain.jpg)]-90 bg-cover text-black flex items-center justify-between px-4 h-16 border-b border-red-200/30 shadow-md transition-all duration-300 ease-in-out">
      {/* Vänster: Logga */}
      <Link href="/">
      ...
      <NavigationMenu className={`md:flex items-center justify-end mx-2 ${menuOpen ? "flex flex-col absolute top-16 right-4 bg-white rounded shadow-lg p-4 z-50" : "hidden md:flex"}`}>
        ...
        >
          <NavigationMenuItem className="flex items-center flex-col sm:flex-row ">
            <Link href="/kontakt">
              <NavigationMenuLink className={navigationMenuTriggerStyle() + " text-white font-bold border-2 bg-[#b697ec] border-black/20 hover:scale-105 transition-all duration-200 ease-in-out text-shadow-2xs"}>
                Kontakta oss
              </NavigationMenuLink>
            </Link>
            <Link href="/om-oss">
              <NavigationMenuLink className={navigationMenuTriggerStyle() + " md:ml-4 mt-2 md:mt-0 text-black border-2 bg-white border-black/20 hover:scale-105 transition-all duration-200 ease-in-out "}>
                Om oss
              </NavigationMenuLink>
            </Link>
          </NavigationMenuItem>
